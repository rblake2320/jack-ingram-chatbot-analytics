    ...
    async def process_request(
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["hours", "location", "contact", "about", "services"]
            ):
                category = next(
                    (k for k in ["hours", "services"] if k in message.lower()), "main"
                )
                dealership_info = self.knowledge.get_dealership_info(category, brand)

                # Cache and return formatted dealership info
                if dealership_info:
                    response = self.format_dealership_response(
                        dealership_info, category
                    )
            ...
            if "date" in message.lower() or "time" in message.lower():
                final_response["response"] = (
                    f"Today's date is {current_time.strftime('%B %d, %Y')} {current_time.strftime('%Z')}"
                )
