...
class APIGateway:
    ...
    async def get_vehicle_data(self, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Combine results
            results["inventory"] = (
                responses[0] if not isinstance(responses[0], Exception) else {}
            )
            results["specs"] = (
                responses[1] if not isinstance(responses[1], Exception) else {}
            )
            results["safety"] = (
                responses[2] if not isinstance(responses[2], Exception) else {}
            )

        return results

    async def get_inventory_data(
        self, session: aiohttp.ClientSession, query: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Get current inventory data"""
        try:
            async with session.get(
                f"{self.base_urls['inventory']}/vehicles", params=query
            ) as response:
    ...
    async def get_car_specs(
        ...
        try:
            async with session.get(
                f"{self.base_urls['car']}/specs", params=query
            ) as response:
                return await response.json()
        except Exception as e:
